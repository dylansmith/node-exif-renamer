...
exifRenamer = {
    ...
    get_exif: function(filepath, callback) {
        ...
        try {
            ...
            if (!result.exif) {
                callback(this.error('EXIF data not found', result), result);
            }
            else {
    ...
    get_metadata: function(filepath, callback) {
        ...
        this.get_exif(src.path, function(err, exif_data) {
            ...
            if (datetime) {
                ...
                return callback(this.error('Cannot resolve datetime: no EXIF datetimes and fallback_ctime=false', metadata));
            }
    ...
    process: function(filepath, template, callback) {
        ...
        // throw if not found
        if (!src.stat) {
            return callback(this.error('path not found: ' + filepath, result));
        }

        // throw if a directory
        if (src.stat.isDirectory() === true) {
            return callback(this.error('path cannot be a directory: ' + filepath, result));
        }

        // throw if not a file
        if (src.stat.isFile() === false) {
            return callback(this.error('path is not a valid file:' + filepath, result));
        }
