...
exifRenamer = {
    ...
    get_metadata: function(filepath, callback) {
        ...
        this.get_exif(src.path, function(err, exif_data) {
            ...
            if (datetime) {
                ...
                datetime = new Date(Math.min(src.stat.birthtime, src.stat.ctime, src.stat.mtime) * 1000);
            }
    ...
    rename_dir: function(dirpath, template, recursiveOrGlob, callback, itemCallback) {
        ...
        itemCallback = itemCallback || _.noop;
        ...
        glob.sync(pattern, {cwd: p}).forEach(function(file) {
            ...
            if (fs.lstatSync(f).isFile()) {
                promises.push(Q.npost(this, 'rename', [f, template]).then(
                    // success
                    function(result) {
                        itemCallback(false, result);
                        return result;
                    },
                    // error
                    function(err) {
        ...
        Q.allSettled(promises).then(function(results) {
            results = _.map(results, function(r) {
                return (r.value instanceof Error && r.value.result) ? r.value.result :  r.value;
            });
